...
def multiGPU_CLIP(sim_encoder, images, prompt_token, prompt=False):
    if prompt_token is not None:
        bs = images.size(0)
        prompt_token = prompt_token.repeat(bs, 1, 1)
    
    if(prompt==True):
        img_embed = sim_encoder(images, (prompt_token))#gives error about arguments given more than required
    else:
        img_embed = sim_encoder(images, None)
...
class AdvQDet(StateModule):
    def __init__(self, arguments, training=False, promptFlag=False,sixteen=False, randomizeprompt=False, path="./acpt/ACPT_checkpointoriginal.pth.tar", prompt_lengthh=20):
        add_prompt_len=prompt_lengthh
        
        print("Path of the checkpoint:", path)
        ...
        if(sixteen==True):
            model, self.preprocess = clip.load("ViT-B/16", device=self.device, prompt_len=add_prompt_len)
        else:
        ...
        self.randomPromptFlag=randomizeprompt
        
      
        
        if(training==False):
            model.eval()
        self.encoder = self.model.module.encode_image
        ...
        max_value = torch.max(self.ind_prompt)
        min_value = torch.min(self.ind_prompt)
        
        
        print("self.ind_prompt Maximum value:", max_value.item())
        print("self.ind_prompt Minimum value:", min_value.item())
        print(self.ind_prompt. shape)
        with torch.no_grad():
            
            if(randomizeprompt==True):
                #self.ind_prompt=self.ind_prompt + (torch.rand_like(self.ind_prompt)*1)
                random_number = random.randint(0, 10)
                self.ind_prompt=(torch.rand_like(self.ind_prompt)*random_number)
                self.ind_prompt=torch.nn.functional.hardtanh(self.ind_prompt, min_val=-33.61, max_val=26.93)
                self.ind_prompt = torch.rand_like(self.ind_prompt) * (24 - (-36)) + (-36)
                """
        ...
        #print("self.ind_prompt Maximum value:", max_value.item())
        #print("self.ind_prompt Minimum value:", min_value.item())
        #print(self.ind_prompt)
        
        #print("prompt encoder: ",self.ind_prompt.shape)
    ...
    def img_preprocess(self, img, normalization=True, pil=True):
        from PIL import Image
        from torchvision.transforms import Compose, Resize, CenterCrop, ToTensor, Normalize
        if(pil==True):
            if len(img.shape) != 3:
                raise Exception("Expected 3D image (C, H, W).")
        def _convert_image_to_rgb(image):
            return image.convert("RGB")
        if(normalization):
        ...
        if(pil):
            ...
            image = preprocess(img).to(self.device)
    
        
        return image
        
        
    def getDigest(self, img, requires_grad=False, preprocess=True):
        """
        Compute the embedding for the given image.
    
        Args:
            img (torch.Tensor): Input image tensor of shape (C, H, W).
            requires_grad (bool): Whether to enable gradient computation for the embedding.
        ...
        if(preprocess==True):
            if len(img.shape) != 3:
                raise Exception("Expected 3D image (C, H, W).")
            #print("(GetDigest Function Value) Before:", img[0][0][0:2] )
            pil_img = transforms.ToPILImage()(img)
            image = self.img_preprocess(pil_img,normalization=True, pil=False).unsqueeze(0).to(self.device)
            #print("(GetDigest Function Value) After:", image[0][0][0][0:2] )
        else: 
            image=img
    
    
        # Compute embedding with or without gradients
        if not requires_grad:
            with torch.no_grad():
                """
                if(self.randomPromptFlag==True):
                    random_number = random.randint(0, 10)
                    self.ind_prompt=(torch.rand_like(self.ind_prompt)*random_number)
                    self.ind_prompt=torch.nn.functional.hardtanh(self.ind_prompt, min_val=-33.61, max_val=26.93)
                    self.ind_prompt = torch.rand_like(self.ind_prompt) * (24 - (-36)) + (-36)
                ...
                embed = multiGPU_CLIP(self.encoder, image, self.ind_prompt, prompt=self.promptFlag).squeeze(0)
        else:
            embed = multiGPU_CLIP(self.encoder, image, self.ind_prompt, prompt=self.promptFlag).squeeze(0)
    ...
    def add(self, img, prediction, preprocess=True):
        #if len(img.shape) != 3:
        #    print("len of img.shape: ", len(img.shape))
        #    raise Exception("expected 3d image")
        encoding = self.getDigest(img, preprocess=preprocess)
        self.cache[encoding] = prediction
    
    def resultsTopk(self, img, k, preprocesss=True):
...
class StatefulClassifier(torch.nn.Module):
    ...
    def forward_single(self, x):
        self.total += 1
        #print("x shape recevied by stateful classifier", x.shape)
...
def init_stateful_classifier(config):
    if config['architecture'] == 'resnet20':
        ...
        model = vit16()
        model.eval()
    elif config['architecture'] == 'skin-cancer':
